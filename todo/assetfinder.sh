...
# urlscan.io
curl --silent "https://urlscan.io/api/v1/search/?q=domain:${domain}" | jq '.results[].task.url' | tr '[:upper:]' '[:lower:]' | uniq
...
curl --silent "https://crt.sh/?q=${domain}&output=json" | jq '.[] | .name_value' | tr '[:upper:]' '[:lower:]' | uniq

# This one is exposed by a company that runs these kind of scans. Newline delimited return, along with IP attached.
curl --silent "https://api.hackertarget.com/hostsearch/?q=${domain}"

# Same deal here, just threat scanning, but doesn't have as much info. Just using for subdomains, since that is useful.
curl --silent "https://www.threatcrowd.org/searchApi/v2/domain/report/?domain=${domain}" | jq '.subdomains | .[]'
